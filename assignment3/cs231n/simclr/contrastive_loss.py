...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = exponential.masked_select(mask).reshape(2 * N, -1)  # [2*N, 2*N-1]
    ...
    pos_pairs = torch.diag(sim_matrix[:N, N:]).reshape(-1, 1) # Opt 1
    # pos_pairs = sim_positive_pairs(out_left, out_right) # Opt 2
    pos_pairs = pos_pairs.repeat(2, 1).to(device) # torch.cat([sim_pairs, sim_pairs], dim=0)
    ...
    numerator = torch.exp(pos_pairs / tau) #.to(device)

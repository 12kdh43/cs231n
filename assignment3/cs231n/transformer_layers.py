...
class MultiHeadAttention(nn.Module):
    ...
    Usage:
      ...
      # self-attention
      data = torch.randn(batch_size, sequence_length, embed_dim) # (N, L, D/H)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        q = self.query(query).reshape(N, S, self.H, D//self.H).transpose(1,2) # (N, H, S, D/H)
        k = self.key(key).reshape(N, T, self.H, D//self.H).transpose(1,2) # (N, H, T, D/H)
        v = self.value(value).reshape(N, T, self.H, D//self.H).transpose(1,2) # (N, H, T, D/H)
        e = torch.matmul(q , k.transpose(2, 3)) / math.sqrt(D//self.H) # (N, H, S, T)

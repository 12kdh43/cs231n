...
def rnn_step_backward(dnext_h, cache):
    ...
    # tanh derv = (1-tanh)(1+tanh)
    dtanh = dnext_h * (1- np.power(next_h, 2)) # (N, H)
    db = np.sum(dtanh, 0) # (H, )
...
def rnn_backward(dh, cache):
    ...
    dprev_h = np.zeros((N, H)) # no previous gradient: init to zero
    dx = np.zeros((N, T, D))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    db = np.zeros(H)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a[:, :3*H] = sigmoid(a[:, :3*H]) # i, f, o gates
    a[:, 3*H:] = np.tanh(a[:, 3*H:]) # g gate
...
def lstm_backward(dh, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, T, H = dh.shape
    _, D = cache[0][0].shape # cache[i] = x, prev_h, prev_c, Wx, Wh, i, f, o ,g, next_c

    dprev_c = dprev_h = np.zeros((N,H)) # no previous gradient: init to zero
    dx = np.zeros((N, T, D))    
    dWx = np.zeros((D, 4*H))
    dWh = np.zeros((H, 4*H))
    db = np.zeros(4*H)
    ...
    for t in range(T-1, -1, -1):
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dprev_h + dh[:, t, :], dprev_c, cache[t])
        dx[:, t, :] += dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    dx_flat /= N
    dx_flat *= mask_flat[:, None] # reshape.(-1, 1)

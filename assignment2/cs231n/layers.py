...
def softmax_loss(x, y):
    ...
    num_train = x.shape[0]
    x_scaled = x - np.max(x, axis = 1).reshape(num_train, 1) # scale for exp() not to explode
    p_scaled = np.exp(x_scaled) / np.sum(np.exp(x_scaled), axis = 1).reshape(num_train ,1)
    loss = -np.sum(np.log(p_scaled[np.arange(num_train), y]))
    loss /= num_train

    p_scaled[np.arange(num_train), y] -= 1
    dx = p_scaled / num_train
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        x_cen = x - x_mu
        x_var = np.var(x, 0) # (D, )
        x_sig_inv = 1 / np.sqrt(x_var + eps) # (D, )
        x_norm = x_cen * x_sig_inv
        out = gamma * x_norm + beta
        cache = (x_cen, x_var, x_sig_inv, x_norm, gamma, eps)

        sample_mean, sample_var, sample_norm = x_mu, x_var, x_norm
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout, 0)
    """
    dx_var = np.sum(dout * gamma * x_cen, 0) * -0.5 * np.power(x_sig_inv, 3)
    dmu = np.sum(dout * gamma * -x_sig_inv, 0) + dx_var * -2 * np.mean(x_cen, 0)
    """
    dx = dout * gamma * x_sig_inv + (np.sum(dout * gamma * x_cen, 0) * -0.5 * np.power(x_sig_inv, 3)) * (2 / N) * x_cen + (np.sum(dout * gamma * -x_sig_inv, 0) + (np.sum(dout * gamma * x_cen, 0) * -0.5 * np.power(x_sig_inv, 3)) * -2 * np.mean(x_cen, 0)) / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    w_row = w.reshape(F, C * HH * WW) # (F, C*HH*WW)
    x_col = np.zeros((N, C * HH * WW, out_h * out_w))
    # x_col (N, C*HH*WW size stretched, H'*W' number of regions) for each obs.
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
      ...
      dx_col[n] = w_row.T.dot(dout_col) # (C*HH*WW, F) * (F, H'*W') = (C*HH*WW, H'*W')
      dw += np.dot(dout_col, x_col[n].T).reshape(w.shape) # (F, H'*W') * (H'*W', C*HH*WW) = (F, C*HH*WW)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
      ...
      out = out_col.reshape(N, C, out_h, out_h)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = x.shape
    x_t = x.reshape(N * G, C // G, H * W).T # (H*W, C//G, N*G)

    x_mu = np.mean(x_t, 0) # mean of (H*W) values for groups -> (C//G, N*G)
    x_cen = x_t - x_mu
    x_var = np.var(x_t, 0)
    x_sig_inv = 1 / np.sqrt(x_var + eps)
    x_norm = x_cen * x_sig_inv
    out = gamma * x_norm.T.reshape(N, C, H, W) + beta 
    cache = (x_cen, x_var, x_sig_inv, x_norm, gamma, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx_norm = (dout * gamma).reshape(size).T
    dout = dout.reshape(size).T

    dx_var = np.sum(dx_norm * x_cen, 0) * -0.5 * np.power(x_sig_inv, 3)
    dmu = np.sum(dx_norm * -x_sig_inv, 0) + dx_var * -2 * np.mean(x_cen, 0)
    dx = dx_norm * x_sig_inv + dx_var * (2 / M) * x_cen + dmu / M

...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if i == 1:
                o, c, names = [X], [], []                  # o[0]: X for initial step
            else: o, c, names = [iter_out], [], []    # o[0]: output from prev iter

            a, a_c = affine_forward(o[-1], self.params['W'+str(i)], self.params['b'+str(i)])
            o.append(a), c.append(a_c), names.append('a'+str(i))

            if self.normalization == "batchnorm":
                n, n_c = batchnorm_forward(o[-1], self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
                o.append(n), c.append(n_c), names.append('n'+str(i))
            if self.normalization == "layernorm":
                n, n_c = layernorm_forward(o[-1], self.params['gamma'+str(i)], self.params['beta'+str(i)], self.bn_params[i-1])
                o.append(n), c.append(n_c), names.append('n'+str(i))

            r, r_c = relu_forward(o[-1])
            o.append(r), c.append(r_c), names.append('r'+str(i))

            if self.use_dropout:
                d, d_c = dropout_forward(o[-1], self.dropout_param)
                o.append(d), c.append(d_c), names.append('d'+str(i))

            outs.update(list(zip(names, o[1:])))    # save except one from prev(init) layer
